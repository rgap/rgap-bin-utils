...
# This one is from https://github.com/andyp123/mp4_to_mp3/
...
def main(indir, outdir):
    ...
    if len(files) == 0:
        exit(
            "Could not find any files to convert that have not already been converted."
        )

    # convert all unconverted files
    for filename in files:
        print(
            "-- converting {0}/{2}.mp4 to {1}/{2}.mp3 --".format(
                indir, outdir, filename
            )
        )
        call(
            [
