...
def getFacebookPageFeedUrl(base_url):
    ...
    fields = (
        "&fields=message,description,link,picture,created_time,type,name,id,"
...
def processFacebookPageFeedStatus(status):
    ...
    status_message = (
        "" if "message" not in status else unicode_decode(status["message"])
    )
    status_description = (
        "" if "description" not in status else unicode_decode(status["description"])
    )
    link_name = "" if "name" not in status else unicode_decode(status["name"])
    status_picture = (
        "" if "picture" not in status else unicode_decode(status["picture"])
    )
    ...
    num_comments = (
        0 if "comments" not in status else status["comments"]["summary"]["total_count"]
    )
...
def scrapeFacebookPageFeedStatus(
    page_id, access_token, since_date, until_date, output_dir
):
    with open(output_dir + "/{}_facebook_statuses.csv".format(page_id), "w") as file:
        ...
        since = (
            ...
            if since_date is not "" and since_date is not None
            else ""
        )
        until = (
            "&until={}".format(until_date)
            if until_date is not "" and since_date is not None
            else ""
        )
