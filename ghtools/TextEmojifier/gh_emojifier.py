...
def main(args):
    ...
    df_emojis_unique = pd.read_csv(
        script_path + "/emojisu_clean_mod.csv"
    )  # especial emojis like country names
    df_emojis["es"] = df_emojis["es"].apply(lambda x: clean_text(x))
    df_emojis_unique["es"] = df_emojis_unique["es"].apply(
        lambda x: clean_text(x, full=False)
    )
